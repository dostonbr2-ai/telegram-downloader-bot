...
def _download_with_ytdlp(url: str) -> Dict[str, Any]:
    ...
    ydl_opts = {
        # Формат: наилучшее видео + аудио размером до 48MB (лимит Telegram Bot API 50MB)
        'format': 'bestvideo[filesize<=48M][ext=mp4]+bestaudio[ext=m4a]/best[filesize<=48M][ext=mp4]/best[filesize<=48M]/best',
        ...
        'postprocessors': [{
            ...
            'preferedformat': 'mp4',
        }] if os.system("ffmpeg -version >/dev/null 2>&1") == 0 else [],
    }
